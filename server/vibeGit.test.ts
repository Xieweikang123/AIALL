...
describe("vibeGit on unborn branch / empty repo", () => {
  ...
  it("gitStatus lists blocked IDE and agent paths for display", async () => {
    ...
    const res = await gitStatus(tempDir);
    expect(res.files.some((f) => f.path.includes(".vs") && f.path.includes("cache.vsidx"))).toBe(true);
    expect(res.files.some((f) => f.path === ".aiall/project-memory.md")).toBe(true);
    expect(res.files.some((f) => f.path === "Program.cs")).toBe(true);
  });
