...
use super::policy::{resolve_run_policy, AgentMode, ResolvePolicyInput};
...
fn evaluate_rust_policy_actual(input: &RustRegressionPolicyInput) -> HashMap<String, Value> {
    ...
    let read_only_tools =
        policy.uses_read_only_tools(is_read_only_agent(&input.mode), input.is_plan_explore);
    ...
    actual.insert(
        "implementFollowUpRun".into(),
        json!(policy.implement_follow_up_run),
    );
    ...
    actual.insert(
        "ultraShortOpenTaskRun".into(),
        json!(policy.ultra_short_open_task_run),
    );
    actual.insert("codeReviewRun".into(), json!(policy.code_review_run));
    actual.insert(
        "sameIssueFollowUpRun".into(),
        json!(policy.same_issue_follow_up_run),
    );
    actual.insert(
...
#[allow(dead_code)]
pub fn run_rust_regression_vectors(
    vectors: &[RustRegressionVector],
) -> Vec<RustRegressionMismatch> {
...
#[allow(dead_code)]
pub fn load_and_run_rust_regression_vectors(
    path: &Path,
) -> Result<Vec<RustRegressionMismatch>, String> {
...
pub fn format_rust_regression_mismatches(mismatches: &[RustRegressionMismatch]) -> String {
    ...
    let mut lines = vec![format!(
        "Rust agent regression: {} mismatch(es)",
        mismatches.len()
    )];
    ...
    for item in mismatches {
        ...
        lines.push(format!(
            "      {}: expected {}, got {}",
            item.field, item.expected, item.actual
        ));
    }
    lines.join("\n")
}
