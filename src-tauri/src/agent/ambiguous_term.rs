...
fn quoted_term_re() -> &'static Regex {
    QUOTED_TERM_RE
        .get_or_init(|| Regex::new(r#"[「『"']([^」』"']{2,16})[」』"']"#).expect("QUOTED_TERM_RE"))
}
...
pub fn has_recent_ambiguity_clarification_offer(
...
pub fn resolve_ambiguous_clarification_terms(input: ResolveAmbiguousInput<'_>) -> Vec<String> {
    ...
    for raw in extract_candidate_terms(&body) {
        ...
        if Regex::new(r"\.(?:vue|ts|tsx|js|jsx|json|md|cs|rs)$")
            .unwrap()
            .is_match(&raw)
        {
            continue;
