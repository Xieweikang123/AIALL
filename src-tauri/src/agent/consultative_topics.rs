...
static PROJECT_OVERVIEW_TOPIC_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(
    || {
        regex::Regex::new(
...
static SESSION_AUDIT_TASK_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
    regex::Regex::new(
...
static GIT_WORKING_TREE_TOPIC_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(
    || {
        regex::Regex::new(
...
static IMPLEMENTATION_STATUS_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(
    || {
        regex::Regex::new(
...
pub fn build_consultative_build_hint() -> &'static str {
   ...
   优先 grep 精确符号；「会不会/是否/做 X 时会不会 Y/准确吗」须沿实际调用关系追到决定结论的条件、调用关系与结果点；禁止只读单个函数或根据目录名称猜测链路。\n\
   准确度/输出质量类：须 read 到实际决定输出的数据、上下文或结果构造处，说明代码里真正注入或产生了什么；禁止用「如果 prompt 包含…」猜测。\n\
...
pub fn build_consultative_resume_hint(behavior_purpose: bool) -> String {
    ...
    "若原始问题为准确度/是否类且证据链尚未覆盖决定结果的代码：须继续 read 相关定义、调用关系、分支与结果构造；禁止「基于已有信息直接回答」或反问用户要不要继续查。".to_string(),
  ];
    if behavior_purpose {
        lines.push(
      "原始问题为用途/作用类：须基于下方已 read 的分支、调用关系或结果构造作答（条件→结果），禁止重复枚举定义或写「可能需要查看引用」。".into(),
    );
    }
...
fn topic_hint(id: &str, prompt: &str) -> String {
    match id {
        ...
        "behavior_contradiction" => {
            super::intent_hints::build_behavior_contradiction_hint().to_string()
        }
        ...
        "step_clarification" => {
            super::intent_hints::build_agent_step_clarification_hint().to_string()
        }
...
fn resolve_config_binding_topic(prompt: &str) -> Option<&'static str> {
    ...
    static ENUM_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
        regex::Regex::new(r"(?:几个|多少|几种|几个选项|有哪些选项|可选值|枚举值|选项列表)").unwrap()
    });
    static DOC_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
        regex::Regex::new(r"(?:官方文档|文档怎么说|查文档|API文档|文档里|spec|规范)").unwrap()
    });
    static CONFIG_CTX_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
        regex::Regex::new(r"(?:字段|属性|参数|配置|选项|枚举|映射|绑定|prop|Props|attribute)")
            .unwrap()
    });
...
fn topic_is_active(
    ...
    match id {
        ...
        "implementation_status" => {
            IMPLEMENTATION_STATUS_RE.is_match(text)
                && history_suggests_active_implementation(history)
        }
...
pub fn resolve_active_consultative_topic_ids(
    ...
    TOPIC_IDS
        .iter()
        .copied()
        .filter(|id| topic_is_active(id, prompt, history, policy))
        .collect()
}
...
mod tests {
    ...
    fn resolves_session_audit_topic_from_policy() {
        ...
        let ids = resolve_active_consultative_topic_ids(
            "",
            None,
            &UserIntent {
