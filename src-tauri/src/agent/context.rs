use super::knowledge_explore::{
    classify_explore_knowledge_intent, explore_intent_uses_knowledge_manifest,
    ExploreKnowledgeIntent,
};
use super::knowledge_manifest::{
    build_knowledge_explore_manifest, build_knowledge_rebuild_hint,
    parse_project_knowledge_frontmatter, PROJECT_KNOWLEDGE_REL,
};
...
async fn read_open_file_snippet(
    project_path: &str,
    open_file_path: &str,
) -> Option<(String, String)> {
    let Ok((resolved, relative)) = crate::paths::resolve_project_path(project_path, open_file_path)
    else {
        return None;
    };
...
async fn build_exploration_archive_prompt_block(
    ...
    for (entry, _) in scored {
        let Ok((resolved, _)) = crate::paths::resolve_project_path(project_path, &entry.path)
        else {
            continue;
        };
...
pub async fn build_context_blocks(input: ContextBuildInput<'_>) -> ContextBlocks {
    ...
    if !input.consultative_ui_appearance_run {
        ...
        if false {
            let knowledge =
                project::read_text_file(input.project_path, PROJECT_KNOWLEDGE_REL).await;
            if knowledge.get("ok").and_then(|v| v.as_bool()) == Some(true) {
                    ...
                    if input.mode == "explore" {
                        if has_body {
                            ...
                            if intent == ExploreKnowledgeIntent::Rebuild {
                                ...
                                if let Some(head) =
                                    meta.git_head.as_deref().filter(|s| !s.trim().is_empty())
                                {
                                    let diff =
                                        git::git_changed_files_since(input.project_path, head)
                                            .await;
                                    if diff.get("ok").and_then(|v| v.as_bool()) == Some(true) {
                                        if let Some(files) =
                                            diff.get("files").and_then(|v| v.as_array())
                                        {
                                ...
                                parts.push(format!(
                                    "\n\n{}",
                                    build_knowledge_explore_manifest(
                                        &body,
                                        &meta,
