...
fn plan_quote_short_edit_re() -> &'static Regex {
  PLAN_QUOTE_SHORT_EDIT_RE
    .get_or_init(|| Regex::new(r"(?:改|删|加|去掉|不要|替换)").expect("PLAN_QUOTE_SHORT_EDIT_RE"))
}

fn execution_continue_re() -> &'static Regex {
  EXECUTION_CONTINUE_RE
    .get_or_init(|| Regex::new(r"(?i)^继续(?:执行|改|做|写|来|完成)(?:吧|一下)?\.?$").expect("EXECUTION_CONTINUE_RE"))
}
...
fn normalize_plan_paths(paths: Vec<String>) -> Vec<String> {
  ...
  unique
    ...
    .filter(|path| {
      ...
      !unique.iter().any(|other| other != *path && other.ends_with(&format!("/{path}")))
    })
    .cloned()
    .collect()
}
...
fn is_plan_executed_after_index(history: &[HistoryMessage], plan_idx: usize) -> bool {
  for msg in history.iter().skip(plan_idx + 1) {
    ...
    if msg.role == "assistant"
      && (msg.content.contains("已写入") || msg.content.contains("write_file") || msg.content.contains("patch_file"))
    {
      return true;
    }
  }
...
pub fn resolve_pending_plan_state(history: Option<&[HistoryMessage]>) -> PendingPlanState {
  ...
  for (i, msg) in history.iter().enumerate() {
    ...
    return PendingPlanState { has_pending_plan: true };
  }
