...
pub fn resolve_explore_turn_budget(is_explore: bool, is_execute_plan: bool, is_plan_explore: bool) -> u32 {
  if is_explore {
...
pub fn build_explore_budget_nudge(consecutive_turns: u32, mode: &str) -> String {
  ...
  format!(
    "【系统提示】已连续 {consecutive_turns} 轮仅探索、尚未修改。\n{action_hint}"
  )
}
...
pub fn build_runtime_tool_failure_recovery_nudge(consecutive_skipped: u32, forced_count: bool) -> String {
  if forced_count {
    format!(
...
pub fn build_grep_hit_vue_read_nudge(vue_files: &[String]) -> String {
  let primary = vue_files.iter().take(2).cloned().collect::<Vec<_>>().join("、");
  format!(
...
pub fn build_patch_failure_completion_retry_nudge(
  ...
  let failed = failed_paths
    ...
    .collect::<Vec<_>>()
    .join("、");
  let failed = if failed.is_empty() { "未知".to_string() } else { failed };
  let success = success_paths
    ...
    .collect::<Vec<_>>()
    .join("、");
  let success = if success.is_empty() { "无".to_string() } else { success };
  format!(
...
pub fn build_premature_completion_retry_nudge(user_reported_failure: bool) -> String {
  ...
  if user_reported_failure {
    lines.push("用户已报告「试了不行/没有效果」——禁止重复宣称成功，须换排查方向或给出可执行验证步骤。".to_string());
  }
  lines.join("")
}
...
pub fn build_code_review_honesty_nudge(user_reported_failure: bool) -> String {
  ...
  if user_reported_failure {
    lines.push("用户近期报告实测失败：优先排查为何无效，勿重复「链路完整」类总结。".to_string());
  }
  lines.join("\n")
}
...
pub fn build_implement_follow_up_hint(quote_position_fix: bool) -> String {
  let mut lines = vec![
    String::new(),
    "【确认执行·须改代码】用户在上文分析或部分实施后要求继续修复/改吧，不是再要一篇分析。".to_string(),
    "最多 1–2 次 read_file 核对目标后即 patch_file/write_file；禁止只分析并反问「要不要修」。".to_string(),
    "禁止输出「请将以下修改应用到…」或只贴代码块让用户手动改；你必须亲自提交 patch。".to_string(),
  ];
  if quote_position_fix {
    lines.push("最多 1 次 read_file 核对上文已定位的目标函数；禁止重复 grep 已讨论过的定位问题。".to_string());
    lines.push("优先改上文已识别的目标文件和函数。".to_string());
  }
  lines.join("\n")
}

pub fn build_automated_bug_fix_hint(verify_script: Option<&str>, include_logic_review: bool) -> String {
...
pub fn claims_write_completion(text: &str) -> bool {
  let t = text.trim().to_lowercase();
  t.contains("已写入") || t.contains("已修改") || t.contains("已创建")
    || t.contains("written") || t.contains("modified") || t.contains("created")
}
...
mod tests {
    ...
    fn test_get_explore_budget_unknown_defaults_to_build() {
        assert_eq!(get_explore_budget("unknown"), INTERACTIVE_EXPLORE_TURN_BUDGET);
    }
    ...
    fn test_get_soft_cap_build() {
        assert_eq!(get_soft_cap("build"), crate::agent::policy::MAX_TOTAL_EXPLORE_TURNS_SOFT);
    }

    #[test]
    fn test_get_soft_cap_unknown_mode() {
        assert_eq!(get_soft_cap("gibberish"), crate::agent::policy::MAX_TOTAL_EXPLORE_TURNS_SOFT);
    }
    ...
    fn test_get_hard_cap_build() {
        assert_eq!(get_hard_cap("build"), crate::agent::policy::MAX_TOTAL_EXPLORE_TURNS);
    }

    #[test]
    fn test_get_hard_cap_unknown_mode() {
        assert_eq!(get_hard_cap("unknown"), crate::agent::policy::MAX_TOTAL_EXPLORE_TURNS);
    }
    ...
    fn test_resolve_explore_turn_budget_execute_plan() {
        assert_eq!(resolve_explore_turn_budget(false, true, false), EXECUTE_PLAN_EXPLORE_TURN_BUDGET);
    }
    ...
    fn test_build_soft_cap_nudge_at_hard_no_nudge() {
        assert_eq!(build_soft_cap_nudge(ASK_MAX_TOTAL_EXPLORE_HARD, "ask"), None);
    }
    ...
    fn test_build_soft_cap_nudge_plan_mode() {
        ...
        assert!(nudge.is_some());
        assert_eq!(build_soft_cap_nudge(PLAN_MAX_TOTAL_EXPLORE_SOFT - 1, "plan"), None);
    }
    ...
    fn test_build_premature_completion_retry_nudge_consistent() {
        assert_eq!(build_premature_completion_retry_nudge_simple(), build_premature_completion_retry_nudge_simple());
    }
    ...
    fn test_claims_premature_completion_negative_long_text() {
        assert!(!claims_premature_completion("已完成所有修改，包括重构和测试。"));
    }
    ...
    fn test_build_grep_empty_recovery_nudge() {
        ...
        assert!(nudge.contains("handler/composable"));
    }
