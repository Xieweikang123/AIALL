...
pub fn record_read_range(
  ...
  prior_ranges.entry(file_key.to_string()).or_default().push(range);
}
...
pub fn is_blocked_grep_after_locate(
  ...
  teleport_body_confirmed && Regex::new(r"(?i)\btransform\b").map(|re| re.is_match(p)).unwrap_or(false)
}
...
pub fn build_search_files_content_query_message(query: &str) -> String {
  format!(
    "错误：search_files 按文件名匹配，「{query}」更像界面可见文案而非文件名。请改用 grep 搜索该文案或 kebab-case class（如 item-meta、panel-list），命中后再 read_file 核对。"
  )
}
...
pub fn build_overly_broad_vision_grep_message(pattern: &str, anchor_quotes: &[String]) -> String {
  let samples: String = anchor_quotes
    ...
    .join("、");
  format!(
    "错误：grep「{pattern}」过宽，易扫出大量无关命中。读图已摘录可见原文 {samples}。请 grep 其中 ≥4 字的片段，或 grep kebab-case class / title= 等结构标识，再 read_file 核对。"
  )
}
...
pub fn build_low_signal_vision_locate_grep_message(pattern: &str) -> String {
  format!(
    "错误：grep「{pattern}」是泛化状态符号，与读图区域定位无关。请 grep kebab-case class（如 *-badge、*-tab）或 template 中与可见标签相邻的片段，命中后 read_file 核对。勿 grep 界面运行时拼接的数字（标签+计数在源码中不存在）。"
  )
}
...
pub fn should_nudge_english_planning(text: &str) -> bool {
  ...
  if lower.starts_with("now let me") || lower.starts_with("let me") {
    let chinese_count = trimmed.chars().filter(|c| ('\u{4e00}'..='\u{9fff}').contains(c)).count();
...
pub fn is_analysis_only_reply_under_force_patch(text: &str) -> bool {
  ...
  if Regex::new(r"(?i)修复方案|以下是具体修改|###\s*修改|建议改造方案").unwrap().is_match(&body)
    && body.contains("```")
  ...
  Regex::new(r"(?i)截图|读图|图已理解|核心问题|根因|getSelection|getClientRects|position:\s*fixed")
    .unwrap()
    .is_match(&body)
...
mod tests {
  ...
  fn runtime_failure_skips_guard_errors() {
    assert!(!is_system_runtime_tool_failure("错误：Ask 模式下不支持 write_file。"));
  ...
  fn runtime_explore_failure_turn_all_failed_with_runtime() {
    ...
    assert!(!is_runtime_explore_failure_turn(&["错误：Ask 模式下不支持 write_file。".into()]));
  }
  ...
  fn force_patch_after_anchor_located() {
    assert!(should_force_patch_after_anchor_located(true, true, false, false));
    assert!(should_force_patch_after_anchor_located(true, false, true, false));
    assert!(!should_force_patch_after_anchor_located(true, false, false, false));
    assert!(!should_force_patch_after_anchor_located(false, true, true, false));
    assert!(should_force_patch_after_anchor_located(false, true, false, true));
  }
  ...
  fn overlapping_read_blocks_third_overlap() {
    ...
    let err = check_overlapping_read("src/foo.ts", read_line_range_from_args(1280, 40), &ranges);
    assert!(err.is_some());
  ...
  fn patch_anchor_in_tool_output() {
    assert!(text_indicates_patch_anchor("async function showQuoteButtonAt(anchor: DOMRect)"));
    assert!(text_indicates_patch_anchor(".quote-floating { position: fixed; }"));
  ...
  fn patch_old_string_must_appear_in_read_slices() {
    ...
    slices.insert("src/foo.ts:1:200".to_string(), ".real { color: red; }\n".to_string());
    assert!(check_patch_old_string_from_reads("src/foo.ts", ".real { color: red; }", &slices, None).is_none());
    let err = check_patch_old_string_from_reads("src/foo.ts", ".fake { gap: 1px; }", &slices, None);
    assert!(err.is_some());
    assert!(err.unwrap().contains("未出现在你对 src/foo.ts 的已读片段中"));
  }

  #[test]
  fn invalidate_read_caches_and_patch_recovery() {
    let mut guard = ToolGuardState {
      read_slice_cache: HashMap::from([("src/foo.ts:1:50".to_string(), "cached".to_string())]),
  ...
  fn overly_broad_vision_grep() {
    let anchors = vec!["多会话同时进行，好实现吗？".to_string(), "今天·14条".to_string()];
    assert!(is_overly_broad_vision_grep("会话", &anchors, &[]));
    assert!(!is_overly_broad_vision_grep("多会话同时进行", &anchors, &[]));
    assert!(!is_overly_broad_vision_grep("session-item", &anchors, &[]));
    let narrative = ["底部有虚线边框的「打开新项目」按钮"];
    assert!(!is_overly_broad_vision_grep("打开新项目", &["项目切换栏".to_string()], &narrative));
    assert!(is_overly_broad_vision_grep("打开新项目", &["项目切换栏".to_string()], &[]));
  }
  ...
  fn alternate_ui_patch_strategy_after_repeated_failures() {
    ...
    assert!(should_nudge_alternate_ui_patch_strategy(&log, "src/foo.vue"));
    assert!(!should_nudge_alternate_ui_patch_strategy(&log[..1], "src/foo.vue"));
  }

  #[test]
  fn claims_success_despite_patch_failures_detects_false_success() {
    assert!(super::claims_success_despite_patch_failures("✅ 两处修改已完成", 1));
    assert!(!super::claims_success_despite_patch_failures("仍需 read 后再 patch", 1));
    assert!(!super::claims_success_despite_patch_failures("✅ 修复完成", 0));
  }

  #[test]
  fn claims_ghost_modification_reply_detects_false_completion() {
    assert!(super::claims_ghost_modification_reply("已完成修改，刷新查看。"));
    assert!(!super::claims_ghost_modification_reply("以上是建议方案，仅供参考。"));
  }
  ...
  fn record_grep_hit_vue_files_tracks_vue_only() {
    ...
    super::record_grep_hit_vue_files(
      &mut guard,
      &["src/Foo.vue".into(), "src/bar.ts".into()],
    );
