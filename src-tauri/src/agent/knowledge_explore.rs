...
fn explore_continue_prompt_re() -> &'static Regex {
    EXPLORE_CONTINUE_PROMPT_RE.get_or_init(|| {
        Regex::new(r"(?i)^请继续探索项目中尚未覆盖的部分").expect("EXPLORE_CONTINUE")
    })
}
...
fn explore_changes_prompt_re() -> &'static Regex {
    EXPLORE_CHANGES_PROMPT_RE.get_or_init(|| {
        Regex::new(r"(?i)^请针对自上次探索以来变更的代码文件").expect("EXPLORE_CHANGES")
    })
}
...
pub fn classify_explore_knowledge_intent(
    prompt: &str,
    has_existing_body: bool,
...
mod tests {
    ...
    fn detects_continue_preset() {
        assert!(is_explore_continue_prompt(
            "请继续探索项目中尚未覆盖的部分，补充知识库"
        ));
    }
    ...
    fn manifest_intents_include_followup_not_rebuild() {
        assert!(explore_intent_uses_knowledge_manifest(
            ExploreKnowledgeIntent::Followup
        ));
        assert!(explore_intent_uses_knowledge_manifest(
            ExploreKnowledgeIntent::Continue
