...
mod tests {
    ...
    fn check_prompt_first_last(lines: &[&str], mode_keyword: &str, first: &str, last: &str) {
        assert!(!lines.is_empty(), "{} prompt should not be empty", mode_keyword);
        assert!(lines[0].contains(mode_keyword), "first line should contain '{}', got: {}", mode_keyword, lines[0]);
        assert_eq!(lines[0], first, "first line mismatch for {} mode", mode_keyword);
        assert_eq!(lines[lines.len() - 1], last, "last line mismatch for {} mode", mode_keyword);
    }

    #[test]
    fn test_build_ask_system_prompt_lines() {
        let lines = build_ask_system_prompt_lines();
        check_prompt_first_last(&lines, "Ask",
            "你是 AIALL 项目 Agent（Ask 模式）。只读问答，不修改任何文件。",
            "- 二元结论需完整正反证据。");
    }
    ...
    fn test_build_plan_system_prompt_lines() {
        ...
        check_prompt_first_last(&lines, "Plan",
            "你是 AIALL 项目架构师（Plan 模式）。你分析项目并输出结构化的修改方案。",
            "- 方案会自动保存到 `.aiall/plans/` 目录。");
    }
    ...
    fn test_build_build_system_prompt_lines() {
        ...
        check_prompt_first_last(&lines, "Build",
            "你是 AIALL 项目 Agent（Build 模式）。你根据需求创建/修改项目代码。",
            "- 修改前后保持文件编码一致。");
    }
    ...
    fn test_all_prompts_contain_tool_sections() {
        assert!(build_ask_system_prompt_lines().iter().any(|l| l.contains("list_dir")));
        assert!(build_plan_system_prompt_lines().iter().any(|l| l.contains("list_dir")));
        assert!(build_explore_system_prompt_lines(false).iter().any(|l| l.contains("web_search")));
        assert!(build_build_system_prompt_lines().iter().any(|l| l.contains("write_file")));
    }
}
