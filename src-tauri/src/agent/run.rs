...
pub async fn agent_run_headless(
    ...
    let channel = Channel::new(move |body| {
        let value = match body {
            InvokeResponseBody::Json(s) => serde_json::from_str(&s).unwrap_or_else(
                |e| json!({ "type": "error", "data": { "message": format!("event json: {e}") } }),
            ),
            InvokeResponseBody::Raw(bytes) => serde_json::from_slice(&bytes).unwrap_or_else(
                |e| json!({ "type": "error", "data": { "message": format!("event raw: {e}") } }),
            ),
        };
        on_event_cb(value);
        Ok(())
...
pub async fn agent_run(
    ...
    emit(
        &channel,
        json!({ "type": "status", "data": { "phase": "starting", "model": request.model } }),
    );
    ...
    if agent_mode == AgentMode::Auto {
        let (resolved_mode, _) =
            policy::resolve_auto_mode(&request.prompt, request.resolved_user_intent.as_ref());
        ...
        emit(
            &channel,
            json!({ "type": "status", "data": { "phase": "auto_mode_resolved", "detail": effective_mode_str } }),
        );
    } else {
    ...
    let is_execute_plan = request
        .run_profile
        .as_ref()
        .map_or(false, |p| p.is_execute_plan());
    let automated_bug_fix = request
        .run_profile
        .as_ref()
        .map_or(false, |p| p.is_auto_bug_fix());
    let web_proxy_url = request
        .web_proxy_url
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty());
    ...
    let resolve_input = policy::ResolvePolicyInput {
        ...
        trigger_source: request
            .run_profile
            .as_ref()
            .and_then(|p| p.trigger_source.clone()),
    ...
    let read_set: std::collections::HashSet<&str> =
        tools::read_only_tool_names().iter().cloned().collect();
    ...
    let tool_names: String = active_tools
        .as_array()
        .map(|a| {
            a.iter()
    ...
    let nudge_mode = if effective_mode_str == "ask"
        || effective_mode_str == "explore"
    ...
    loop {
        if run_state.segment.actual_turns >= run_state.segment.max_turns {
            ...
            if read_only_segment_cap || effective_mode_str == "explore" || is_plan_explore {
                let explore_turns = run_state
                    .consultative
                    .total_read_tool_calls
                    .max(crate::agent::EXPLORE_MAX_TOTAL_EXPLORE_SOFT);
                let cap_nudge = if effective_mode_str == "explore" {
                    ...
                    crate::agent::build_plan_segment_cap_nudge(
                        run_state.segment.actual_turns,
                        run_state.consultative.total_read_tool_calls,
                    )
                } else {
                    crate::agent::build_consultative_segment_cap_nudge(
                        run_state.segment.actual_turns,
                        run_state.consultative.total_read_tool_calls,
                    )
                };
                run_state
                    .messages
                    .push(json!({ "role": "system", "content": cap_nudge }));
            ...
            "content": crate::agent::build_plan_force_answer_nudge(run_state.consultative.total_read_tool_calls)
                ...
                run_state.segment.max_turns =
                    extend_segment_max_turns(run_state.segment.actual_turns, segment_budget);
                run_state.messages.push(json!({
          "role": "system",
        ...
        if is_cancelled(&cancel) {
            if effective_mode_str == "explore"
                && !run_state.preflight_state.explore_abort_grace_turn_active
            {
        ...
        run_state
            .tool_guard
            .note_vision_assistant_text(&assistant_text);
        ...
        if !assistant_text.trim().is_empty() && is_final {
            emit(
                &channel,
                json!({ "type": "message", "data": { "text": assistant_text } }),
            );
        }
            ...
            let outcome = handle_final_turn(
                &mut FinalizeTurnParams {
                    ...
                    target_files: request
                        .run_profile
                        .as_ref()
                        .and_then(|p| p.target_files.clone()),
                    ...
                    ambiguous_term_clarification_pending: run_state
                        .ambiguous_term_clarification_pending,
                    ambiguous_term_clarification_terms: &run_state
