...
use super::context_limits::{MAX_TOOL_RESULT_MODEL_CHARS, SOFT_COMPACT_CONTEXT_CHARS};
...
pub fn compact_messages_for_model(messages: &[Value], max_context_chars: usize) -> CompactResult {
    ...
    for &index in tool_indexes.iter().take(compressible) {
        let raw = result[index]
            ...
            .unwrap_or("")
            .to_string();
        let line_hint = LINE_HINT_RE.find(&raw).map(|m| m.as_str()).unwrap_or("");
...
mod tests {
    ...
    fn compresses_older_tool_outputs_when_total_context_is_too_large() {
        ...
        assert!(compacted.messages[2]["content"]
            ...
            .unwrap()
            .contains("已压缩"));
        assert!(compacted.messages[3]["content"]
            .as_str()
            .unwrap()
            .contains("lines 201-400"));
        assert!(compacted.messages[4]["content"]
            .as_str()
            .unwrap()
            .contains("lines 401-600"));
    }
