...
pub(crate) fn build_segment_continue_nudge(
    completed_turn: u32,
    segment_index: u32,
    mode: &str,
) -> String {
...
pub(crate) fn build_agent_turns_low_nudge(
    ...
    format!("【系统提示】剩余约 {remaining} 轮（当前第 {turn}/{max_turns} 轮）。\n{action_hint}")
}
...
pub(crate) fn filter_read_only_tools(
    all_tools: &Value,
    read_set: &std::collections::HashSet<&str>,
) -> Value {
...
pub(crate) fn emit_aborted_done(channel: &Channel<Value>, written_files: &[String], turns: u32) {
    emit(
        channel,
        json!({ "type": "status", "data": { "phase": "aborted" } }),
    );
    emit(
        channel,
        json!({
...
mod tests {
    ...
    fn filter_read_only_tools_selects_read_tools() {
        ...
        let read_set: std::collections::HashSet<&str> = ["read_file", "grep"].into_iter().collect();
    ...
    fn filter_write_only_tools_selects_write_tools() {
        ...
        assert_eq!(
            names,
            vec!["write_file", "patch_file", "delete_file", "run_command"]
        );
    }
