...
use super::ambiguous_term::{
    looks_like_clarification_question, looks_like_premature_plan_or_scaffold,
};
...
pub(crate) fn handle_final_turn(
        ...
        })
    {
        ...
        params
            .messages
            .push(json!({ "role": "system", "content": nudge }));
        emit(
            params.channel,
            json!({
              "type": "status",
    ...
    if !params.is_read_only_run
        ...
        && claims_success_despite_patch_failures(
            params.assistant_text,
            params.patch_failure_log.len(),
        )
    {
