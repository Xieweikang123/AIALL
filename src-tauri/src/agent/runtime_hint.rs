...
pub async fn detect_project_runtime_profile(project_root: &str) -> ProjectRuntimeProfile {
  ...
  let has_electron = Path::new(project_root).join("electron").exists() || Path::new(project_root).join("electron.vite.config.ts").exists();
  ...
  if verify_scripts.is_empty() {
    if Path::new(project_root).join("tsconfig.json").exists() || Path::new(project_root).join("tsconfig.app.json").exists() {
      if Path::new(project_root).join("node_modules/.bin/vue-tsc").exists() {
        verify_scripts.push("npx vue-tsc --noEmit".to_string());
      } else if Path::new(project_root).join("node_modules/.bin/tsc").exists() {
  ...
  ProjectRuntimeProfile { has_desktop_shell, web_dev_script: web_dev, desktop_dev_script: desktop_dev, verify_script, verify_scripts }
}
...
pub fn resolve_verify_scripts(scripts: &[String]) -> Vec<&str> {
  let default_order: &[&str] = &["npm run typecheck", "npm run check", "npm run lint", "npm run test"];
  default_order.iter().filter(|s| scripts.iter().any(|x| x == *s)).copied().collect()
}
...
mod tests {
    ...
    fn test_build_shell_awareness_hint_windows() {
        ...
        assert!(hint.contains("Windows PowerShell"), "expected windows shell hint, got: {}", hint);
    }
