...
fn strip_vision_marker(text: &str) -> String {
  VISION_INTERNAL_MARKER_RE.replace_all(text, "").trim().to_string()
}
...
pub fn should_block_consultative_vision_locate_finalize(
  ...
  if appearance_prompt
    && is_speculative_style_answer(params.reply_text)
    && consultative_needs_grep_hit_vue_read(params.grep_hit_vue_files, params.consultative_read_paths)
  {
    return true;
  }
...
pub fn build_consultative_ui_appearance_retry_hint(vue_files: &[String]) -> String {
  let file_hint = if vue_files.is_empty() {
    ...
    format!(
      ...
      vue_files.iter().take(2).cloned().collect::<Vec<_>>().join("、")
    )
  };
  [
...
pub fn build_vision_consultative_locate_retry_hint(anchor_quotes: &[String]) -> String {
  let anchor_hint = if anchor_quotes.is_empty() {
    "请 grep 读图描述中的可见文案（≥4 字）或 kebab-case class。".to_string()
  } else {
    format!(
...
mod tests {
  ...
  fn should_bypass_vision_first_turn_for_locate_questions() {
    ...
    assert!(!should_bypass_vision_first_turn(0, true, "知道是哪儿的按钮吗？"));
    assert!(!should_bypass_vision_first_turn(1, false, "知道是哪儿的按钮吗？"));
  }
  ...
  fn is_deferred_locate_reply_detects_postponed_answers() {
    assert!(is_deferred_locate_reply("需要在下一轮通过搜索按钮文案来精确确认。"));
    assert!(!is_deferred_locate_reply("位于 FilePanel.vue 的 session-action-btn。"));
  }
  ...
  fn blocks_speculative_style_without_css_read() {
    ...
    let ok_input = ConsultativeVisionFinalizeInput {
      ...
      reply_text: "`.project-history-dropdown { background: var(--bg-primary); }` 为实色，不透明。",
