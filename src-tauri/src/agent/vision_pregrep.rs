...
pub fn format_vision_anchor_pgrep_block(patterns: &[String], matches: &[GrepMatch]) -> String {
  ...
  if matches.is_empty() {
    return [
      header,
      "（无匹配）".to_string(),
      "下一轮请改 grep kebab-case class、title= 或更短的可见片段；勿猜组件路径。".to_string(),
    ]
    .join("\n");
  }
  ...
  let file_list = if unique_files.len() > 8 {
    format!("{}…", unique_files.iter().take(8).cloned().collect::<Vec<_>>().join("、"))
  } else {
    unique_files.join("、")
  };
  [
...
pub async fn run_vision_anchor_pgrep(
  ...
  for pattern in &patterns {
    if let Ok(result) = grep_in_project(project_path, pattern, VISION_ANCHOR_PREFGREP_MAX_MATCHES).await {
      for m in result {
...
mod tests {
  ...
  fn select_patterns_filters_overly_broad() {
    ...
    assert!(patterns.iter().any(|p| p.contains("多会话") || p.contains("会话同时")));
  }
