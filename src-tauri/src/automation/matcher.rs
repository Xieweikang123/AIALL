...
pub async fn find_template_match(
  ...
  tokio::task::spawn_blocking(move || {
    find_template_match_sync(&screen, &template, &opts)
  })
  .await
...
fn find_template_match_sync(
  ...
  let screen_img = image::load_from_memory(screen_png).map_err(|e| format!("解码截屏 PNG 失败: {e}"))?;
  let template_img = image::load_from_memory(template_png).map_err(|e| format!("解码模板 PNG 失败: {e}"))?;
...
fn edge_strength(tpl: &[u8], tw: usize, th: usize, x: usize, y: usize, min_alpha: u8) -> f64 {
  let i = (y * tw + x) * 4;
  if tpl[i + 3] < min_alpha { return -1.0; }
  let r = tpl[i] as f64;
  ...
  for &(dx, dy) in &[(-1i32,0),(1,0),(0,-1),(0,1)] {
    let nx = x as i32 + dx;
    let ny = y as i32 + dy;
    if nx < 0 || nx >= tw as i32 || ny < 0 || ny >= th as i32 { continue; }
    let j = (ny as usize * tw + nx as usize) * 4;
    if tpl[j + 3] < min_alpha { continue; }
    s += (r - tpl[j] as f64).abs() + (g - tpl[j+1] as f64).abs() + (b - tpl[j+2] as f64).abs();
  }
  s
}
...
fn build_smart_probes(tpl: &[u8], tw: usize, th: usize) -> (Vec<usize>, Vec<usize>) {
  ...
  for y in 0..th { for x in 0..tw {
    ...
    if e >= 0.0 { scored.push((x,y,e)); }
  }}
  scored.sort_by(|a,b| b.2.partial_cmp(&a.2).unwrap_or(std::cmp::Ordering::Equal));

  if scored.is_empty() { return (vec![0], vec![0]); }
  ...
  for &(x,y,_) in &scored {
    if x != 0 || y != 0 {
      ox.push(x); oy.push(y); used.push((x,y));
      break;
    }
  }
  ...
  for &(x,y,_) in &scored {
    if ox.len() >= max_pts { break; }
    if used.contains(&(x,y)) { continue; }
    if used.iter().any(|&u| cheby((x,y), u) < 2) { continue; }
    used.push((x,y)); ox.push(x); oy.push(y);
  }
...
fn rgba_exact_at(screen: &[u8], sw: usize, tpl: &[u8], tw: usize, px: usize, py: usize, tx: usize, ty: usize) -> bool {
  let si = ((py+ty) * sw + (px+tx)) * 4;
  let ti = (ty * tw + tx) * 4;
  screen[si..si+4] == tpl[ti..ti+4]
}

fn full_rgba_score(screen: &[u8], sw: usize, sh: usize, tpl: &[u8], tw: usize, th: usize, px: usize, py: usize) -> f64 {
  if px + tw > sw || py + th > sh { return 0.0; }
  let n = tw * th;
  if n == 0 { return 0.0; }
  let ok: usize = (0..th).flat_map(|ty| {
    (0..tw).filter_map(move |tx| {
      if rgba_exact_at(screen, sw, tpl, tw, px, py, tx, ty) { Some(1) } else { None }
    })
  }).count();
  ok as f64 / n as f64
}

fn match_rgba(screen: &[u8], sw: usize, sh: usize, tpl: &[u8], tw: usize, th: usize, opts: &MatchOptions) -> Result<MatchHit, String> {
  let (ox, oy) = build_smart_probes(tpl, tw, th);
  if ox.is_empty() { return Err("模板无有效探针点".into()); }
  ...
  for py in 0..=(sh - th) {
    for px in 0..=(sw - tw) {
      let pass = (0..ox.len()).all(|k| rgba_exact_at(screen, sw, tpl, tw, px, py, ox[k], oy[k]));
      if !pass { continue; }
      accept += 1;
      if reservoir.len() < opts.reservoir_size {
        ...
        if j < opts.reservoir_size { reservoir[j] = (px,py); }
      }
    }
  }
  ...
  let (best_x, best_y, best_score) = reservoir.iter().fold((0,0,-1.0f64), |(bx,by,bs), &(px,py)| {
    let s = full_rgba_score(screen, sw, sh, tpl, tw, th, px, py);
    if s > bs { (px,py,s) } else { (bx,by,bs) }
  });

  if best_score < opts.min_score || best_score < opts.min_score.min(0.95) {
    return Err(format!("未找到足够相似的区域（RGBA {:.3}，要求≥{}）", best_score, opts.min_score));
  }

  Ok(MatchHit {
    top_left_x: best_x as i32, top_left_y: best_y as i32,
...
fn rgb_fuzzy_score(screen: &[u8], sw: usize, tpl: &[u8], tw: usize, px: usize, py: usize, probes: &[(usize,usize)]) -> f64 {
  let mut diff = 0.0f64;
  for &(tx,ty) in probes {
    let si = ((py+ty) * sw + (px+tx)) * 4;
    let ti = (ty * tw + tx) * 4;
...
fn full_rgb_score(screen: &[u8], sw: usize, tpl: &[u8], tw: usize, th: usize, px: usize, py: usize) -> f64 {
  if px + tw > sw { return 0.0; }
  let mut diff = 0.0f64;
  let mut count = 0usize;
  for ty in 0..th { for tx in 0..tw {
    let ti = (ty * tw + tx) * 4;
    if tpl[ti+3] < 16 { continue; }
    let si = ((py+ty) * sw + (px+tx)) * 4;
    diff += (screen[si] as f64 - tpl[ti] as f64).abs();
    diff += (screen[si+1] as f64 - tpl[ti+1] as f64).abs();
    diff += (screen[si+2] as f64 - tpl[ti+2] as f64).abs();
...
fn build_rgb_probes(tpl: &[u8], tw: usize, th: usize, max_n: usize) -> Vec<(usize,usize)> {
  ...
  for &(x,y,_) in &scored {
    if out.len() >= max_n { break; }
    if !out.contains(&(x,y)) { out.push((x,y)); }
  }
  if out.is_empty() { out.push((tw/2, th/2)); }
  out
}

fn match_rgb_multiscale(screen: &[u8], sw: usize, sh: usize, tpl: &[u8], tw: usize, th: usize, tpl_img: &image::RgbaImage, opts: &MatchOptions) -> Result<MatchHit, String> {
  ...
  for &s in &scales {
    ...
    if ntw as usize > sw || nth as usize > sh { continue; }

    let tpl_data = if ntw as usize != tw || nth as usize != th {
      let r = image::imageops::resize(tpl_img, ntw, nth, image::imageops::FilterType::Lanczos3);
      r.into_raw()
    } else {
      tpl.to_vec()
    };
        ...
        if !near {
          ...
          if let Some(idx) = pos {
            if idx < cand_n {
              ...
              if candidates.len() > cand_n { candidates.pop(); }
            }
    ...
    for &(cx,cy,_) in &candidates {
      ...
      for y in y0..=y1 { for x in x0..=x1 {
        ...
        if sc > opts.tolerant_min_score {
          ...
          if improved {
            let algo = if (s - 1.0).abs() < 1e-6 { "rgb_tolerant_sparse_sad" } else { "rgb_tolerant_multiscale_sad" };
    ...
    if let Some(ref b) = best {
      if (s - 1.0).abs() < 1e-6 && b.2 >= early_accept { break; }
    }
  }

  best.map(|(bx,by,sc,algo)| MatchHit {
    top_left_x: bx, top_left_y: by,
