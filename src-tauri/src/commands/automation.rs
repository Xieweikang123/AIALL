...
async fn write_icon_template_store(store_path: &Path, store: &mut IconTemplateStoreFile) -> Result<(), String> {
...
#[tauri::command]
pub async fn automation_find_template(screen_png: String, template_png: String, min_score: Option<f64>) -> Value {
  let screen_bytes = match base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &screen_png) {
    Ok(b) => b,
    Err(e) => return json!({ "ok": false, "error": format!("截屏图片 Base64 解码失败: {e}") }),
  };
  let template_bytes = match base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &template_png) {
    Ok(b) => b,
    Err(e) => return json!({ "ok": false, "error": format!("模板图片 Base64 解码失败: {e}") }),
  };
...
pub async fn automation_open_by_template(template_id: String) -> Value {
  ...
  let hit_obj = hit_value.get("hit").and_then(|v| v.as_object()).cloned().unwrap_or_default();
  ...
  let top_left_x = hit_obj.get("topLeftX").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
  let top_left_y = hit_obj.get("topLeftY").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
  let match_algo = hit_obj.get("matchAlgorithm").and_then(|v| v.as_str()).unwrap_or("").to_string();
...
pub async fn automation_test_icon_template(template_id: String) -> Value {
  ...
  let template_png = match base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &png_b64) {
    Ok(d) => d,
    Err(e) => {
      return json!({
        "ok": false,
...
#[tauri::command]
pub async fn automation_test_match(template_png_b64: String, screen_png_b64: Option<String>) -> Value {
  ...
  let template_png = match base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &template_png_b64) {
    Ok(d) => d,
...
pub async fn icon_templates_save(payload: UpsertIconTemplatePayload) -> Value {
  ...
  let aliases = payload
    ...
    .take(32)
    .collect::<Vec<_>>();
  let note = payload.note.unwrap_or_default().trim().chars().take(500).collect::<String>();
  ...
  let item = IconTemplateItem {
    ...
    note,
    image_file,
    created_at: prev.as_ref().map(|p| p.created_at.clone()).unwrap_or_else(|| now.clone()),
    updated_at: now,
  };
