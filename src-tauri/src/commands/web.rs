...
#[tauri::command]
pub async fn web_extract(url: String, mode: Option<String>, limit: Option<u32>, proxy_url: Option<String>) -> Value {
...
async fn fetch_discourse_latest(url: &str, limit: u32) -> Value {
  ...
  let parsed: Value = match serde_json::from_str(&raw_text) {
    Ok(v) => v,
    Err(_) => return json!({ "ok": false, "status": 200, "error": "latest.json 解析失败", "rawText": raw_text }),
  };

  let topics = parsed["topic_list"]["topics"].as_array().cloned().unwrap_or_default();
  let lines: Vec<String> = {
    ...
    for topic in topics.iter().take(limit as usize) {
      ...
      if !created.is_empty() { v.push(format!("  创建：{created}")); }
      if !last_posted.is_empty() { v.push(format!("  最新回复：{last_posted}")); }
    }
    v
  };
