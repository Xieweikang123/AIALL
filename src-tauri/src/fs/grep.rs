...
pub async fn grep_in_project(project_root: &str, pattern: &str, max_matches: usize) -> Result<Vec<GrepMatch>, String> {
  ...
  for line in output.lines() {
    ...
    if let Some(caps) = RG_LINE_RE.captures(line) {
      ...
      matches.push(GrepMatch {
        ...
        text: caps.get(3).map(|m| m.as_str()).unwrap_or("").trim().chars().take(200).collect(),
      });
    }
  }
  matches
...
async fn grep_in_project_node(root: &str, pattern: &str, max_matches: usize) -> Result<Vec<GrepMatch>, String> {
  let regex = Regex::new(pattern).or_else(|_| {
    Regex::new(&regex::escape(pattern))
  }).map_err(|e| e.to_string())?;
  let mut matches = Vec::new();
  walk_grep(Path::new(root), Path::new(root), &regex, 0, max_matches, &mut matches).await;
  Ok(matches)
}
...
async fn walk_grep(
  ...
  while let Ok(Some(entry)) = entries.next_entry().await {
    ...
    let full = entry.path();
    if is_directory {
      Box::pin(walk_grep(root, &full, regex, depth + 1, max_matches, matches)).await;
      continue;
    }
    let relative = full.strip_prefix(root).unwrap_or(&full).to_string_lossy().replace('\\', "/");
    if should_skip_grep_relative(&relative) {
      continue;
    }
    ...
    let Ok(content) = fs::read_to_string(&full).await else { continue };
    for (i, line) in content.lines().enumerate() {
      ...
      if regex.is_match(line) {
        matches.push(GrepMatch {
          ...
          relative: full.strip_prefix(root).unwrap_or(&full).to_string_lossy().replace('\\', "/"),
          line: (i + 1) as u32,
