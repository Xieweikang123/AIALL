...
pub async fn search_files_impl(dir_path: &str, query: &str, max_results: usize) -> Result<Vec<FileEntry>, String> {
...
async fn walk(
  ...
  while let Ok(Some(entry)) = entries.next_entry().await {
    ...
    if is_directory {
      Box::pin(walk(root, &entry.path(), query, depth + 1, max_results, results)).await;
    }
  }
}
