...
pub async fn create_item_impl(path: &str, is_directory: bool, content: Option<&str>) -> Result<String, String> {
  let p = Path::new(path);
  if is_directory {
    ...
    fs::write(p, content.unwrap_or("")).await.map_err(|e| e.to_string())?;
  }
  Ok(path.to_string())
}
...
pub async fn delete_item_impl(path: &str) -> Result<String, String> {
  let p = Path::new(path);
  let meta = fs::metadata(p).await.map_err(|_| "文件或目录不存在".to_string())?;
  if meta.is_dir() {
