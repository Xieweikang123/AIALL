...
const GIT_LOG_FORMAT: &str = "%x1e%H%x1f%h%x1f%an%x1f%ai%x1f%d%x1f%P%x1f%B%x00";
...
pub async fn git_log(
    ...
    if branch.filter(|value| !value.is_empty()).is_none() && all_branches {
        args.push("--all");
    }
    ...
    if let Some(value) = author.filter(|value| !value.is_empty()) {
        args.push("--author");
        args.push(value);
    }
    if let Some(value) = since.filter(|value| !value.is_empty()) {
        args.push("--since");
        args.push(value);
    }
    if let Some(value) = until.filter(|value| !value.is_empty()) {
        args.push("--until");
        args.push(value);
    }
    if let Some(value) = branch.filter(|value| !value.is_empty()) {
        args.push(value);
    }
    if let Some(value) = path.filter(|value| !value.is_empty()) {
        args.push("--");
        args.push(value);
    }
...
pub(crate) fn parse_git_log(stdout: &str) -> Vec<GitLogEntry> {
    ...
    for block in stdout.split('\x1e').filter(|b| !b.trim().is_empty()) {
        ...
        let lines: Vec<Vec<&str>> = file_str
            .lines()
        ...
        for cols in &lines {
            if cols.len() >= 3
                && (cols[0].trim() == "-" || cols[0].trim().parse::<u32>().is_ok())
                && (cols[1].trim() == "-" || cols[1].trim().parse::<u32>().is_ok())
            {
                stats.insert(
                    cols[2].trim().to_string(),
                    (
                        cols[0].trim().parse().unwrap_or(0),
        ...
        for cols in &lines {
            ...
            let (additions, deletions) = stats
                .get(cols.get(1).copied().unwrap_or("").trim())
                .copied()
                .unwrap_or((0, 0));
        ...
        let (parents, message_start) = if parts.len() >= 7 {
            let parents = parts[5]
                .split_whitespace()
        ...
        entries.push(GitLogEntry {
            ...
            message: parts[message_start..].join("\x1f").trim().to_string(),
            parents,
            files,
            refs,
        });
