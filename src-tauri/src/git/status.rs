...
fn is_unmerged_status(idx: char, wt: char) -> bool {
    matches!((idx, wt), ('U', _) | (_, 'U') | ('A', 'A') | ('D', 'D'))
}
...
pub async fn git_status(project_root: &str) -> GitStatusResult {
    ...
    let head_commit = git_exec(project_root, &["rev-parse", "HEAD"])
        ...
        .map(|o| o.stdout.trim().to_string())
        .unwrap_or_default();
    match git_exec(project_root, &["status", "--porcelain=v1", "-z", "-uall"]).await {
        Ok(out) => {
...
pub async fn git_changed_files_since(project_root: &str, since_commit: &str) -> serde_json::Value {
