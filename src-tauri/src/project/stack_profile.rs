...
fn detect_from_package_json(project_root: &Path, profile: &mut ProjectStackProfile) {
        ...
        .any(|d| d == "vue" || d.starts_with("@vue/"))
    {
        profile.frameworks.push(
            if vue_version.starts_with('3') || vue_version.contains("^3") {
                "vue3".to_string()
            } else {
                "vue".to_string()
            },
    ...
    if dep_names
        .iter()
        .any(|d| d.contains("tauri") || d.starts_with("@tauri-apps/"))
    {
    ...
    if dep_names
        .iter()
        .any(|d| *d == "typescript" || d.starts_with("@types/"))
    {
        profile.languages.push("typescript".to_string());
    }
    if dep_names
        .iter()
        .any(|d| matches!(d.as_str(), "node-cron" | "cron" | "bull" | "agenda"))
    {
...
mod tests {
    ...
    fn parity_project_context_payload_from_env() {
        ...
        let mut actual =
            build_minimal_project_context_payload(&root.replace('\\', "/"), &profile, &routes);
        ...
        assert_eq!(
            actual, expected,
            "Rust payload must match Node node-payload.json"
        );
    }
}
