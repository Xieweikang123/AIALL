use crate::fs::{should_list_directory_entry, is_text_extension};
use serde::Serialize;
...
static TS_PATTERNS: &[(&str, &str)] = &[
  (r"(?m)(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)", "function"),
  (r"(?m)(?:export\s+)?class\s+([A-Za-z_$][\w$]*)", "class"),
  (r"(?m)(?:export\s+)?interface\s+([A-Za-z_$][\w$]*)", "interface"),
  (r"(?m)(?:export\s+)?type\s+([A-Za-z_$][\w$]*)", "type"),
  (r"(?m)(?:export\s+)?enum\s+([A-Za-z_$][\w$]*)", "enum"),
  (r"(?m)(?:export\s+)?const\s+([A-Za-z_$][\w$]*)\s*=", "const"),
  (r"(?m)(?:export\s+)?(?:let|var)\s+([A-Za-z_$][\w$]*)\s*=", "variable"),
];

static RUST_PATTERNS: &[(&str, &str)] = &[
  (r"(?m)^(?:pub\s+)?(?:async\s+)?fn\s+([A-Za-z_][A-Za-z0-9_]*)", "function"),
  (r"(?m)^(?:pub\s+)?struct\s+([A-Za-z_][A-Za-z0-9_]*)", "struct"),
...
pub fn project_symbol_search(project_path: &str, query: &str, max_results: usize) -> Vec<SymbolEntry> {
...
mod tests {
  ...
  fn search_ranks_exact_match_higher() {
    ...
    .unwrap();
    let results = project_symbol_search(dir.to_str().unwrap(), "formatPendingApprovalLabel", 10);
