...
pub async fn browser_extract_inner(
    url: &str,
    proxy_url: Option<&str>,
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let browser =
            headless_chrome::Browser::new(browser_launch_options(proxy_ref)).map_err(|e| {
                format!("启动 Chrome 失败: {e}。请确认已安装 Chrome/Chromium 并加入 PATH。")
            })?;
        let tab = browser
            .new_tab()
