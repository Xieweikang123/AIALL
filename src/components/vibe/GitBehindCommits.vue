<template>
  <div v-if="behind > 0" class="git-behind-section">
    ...
    <div v-if="open" class="git-behind-list">
      ...
      <div v-for="entry in commits" :key="entry.hash" class="git-behind-item">
        <div class="git-behind-entry-head">
          ...
          <span class="git-behind-msg" :title="entry.message">{{ entry.message }}</span>
        </div>
        <div class="git-behind-meta">
          <span class="git-behind-author">{{ entry.author }}</span>
          <span class="git-behind-date">{{ formatDate(entry.date) }}</span>
          <span class="git-behind-files">{{ entry.files.length }} 文件</span>
        </div>
      </div>
    </div>
  </div>
...
.git-behind-author { color: rgba(139, 148, 158, 0.7); }
</style>
