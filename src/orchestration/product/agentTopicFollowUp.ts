...
const SHORT_TOPIC_FOLLOW_UP_RE =
  /^(?:(?:需要|要不要|是否|还得|还要|值得|可以|那)?[^。！!\n]{0,24}(?:吗|呢|对不对)[？?]?|(?:优化|改进|调整)(?:吧|一下|下)?[。！!]?)\s*$/;
...
const REVIEW_REPORT_RE =
  /(?:审计报告|评估报告|排查报告|改进建议|综合评分|工具调用准确性|上下文理解|回复结构)/;

const TOPIC_ANCHOR_HINT = [
  "【延续上一轮话题】上一条助手消息是在解释/答疑（非新任务安排）。",
  "用户本条是短追问，必须围绕该解释继续讨论，判断「是否需要优化/改进」指上条所涉机制、体验或流程；若上文是审计/评估报告，则优先针对报告里的系统行为、流程和规则，而不是示例代码片段。",
...
export function buildImmediateTopicFollowUpHint(
  ...
  if (!EXPLANATORY_ASSISTANT_RE.test(lastAssistantContent) && !REVIEW_REPORT_RE.test(lastAssistantContent)) return prompt;
