...
export function buildReplyAccuracyHint(): string {
  return [
    "事实与准确度（通用）：",
    "1. 机制事实结论（怎么工作、实际走哪条路径、是否会产生某结果）：先拆出结论涉及的定义、调用关系、输入、条件分支与结果；按代码实际结构追踪，不强制固定入口、层数或中间层；继续追到直接决定结论的最后证据，例如返回值、输出、异常、状态、事件、缓存结果或外部副作用，禁止把存在的同名 API、类型字段或注释当成实际使用路径。",
    "2. 二元结论（会/不会、是/不是）及因果解释：grep 命中后须 read 完整函数体、直接调用方和会决定结果的分支；确认所有相关结果路径及其前提，把实际执行路径与旁路 API 区分开，有完整代码证据再作答，证据不足明确说不确定。",
